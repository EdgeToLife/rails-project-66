  ...
  def perform(check, user_id)
    begin
      ...
      if check.repository.language == 'javascript'
        ...
        stdout, exit_status = Open3.popen3("npx eslint #{download_path} --no-eslintrc --config #{eslintrc_path} -f json") { |_stdin, stdout, _stderr, wait_thr| [stdout.read, wait_thr.value] }
      elsif check.repository.language == 'ruby'
        stdout, exit_status = Open3.popen3("rubocop #{download_path} --format json") { |_stdin, stdout, _stderr, wait_thr| [stdout.read, wait_thr.value] }
      end
